...
export class RecipeScraperService {
   ...
   */
  private extractIngredients(recipe: any): Ingredient[] {
   ...
   */
  private parseIngredient(ingredientStr: string): Ingredient | null {
   ...
   */
  private extractFromHtml(html: string): ScrapedRecipe | null {
    try {
      const $ = cheerio.load(html);
